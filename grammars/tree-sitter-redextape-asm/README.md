...
credentials.** Re-measured 2026-08-26, unchanged from what the sibling READMEs record:
...
$ curl -s -o /dev/null -w 'HTTP %{http_code}, %{size_download} bytes\n' \
    'https://git.daveynet.xyz/davey/redextape/info/refs?service=git-upload-pack'
HTTP 200, 0 bytes

$ GIT_TERMINAL_PROMPT=0 git -c credential.helper= ls-remote https://git.daveynet.xyz/davey/redextape
   (exit 0, zero bytes of output)
```

`forge.daveynet.xyz` refuses anonymous access outright, which is the honest failure.
`git.daveynet.xyz` is the **SSH** clone host; over HTTPS it answers the ref advertisement with **HTTP
200 and a zero-byte body**, which git reads as *a repository with no refs* — `ls-remote` exits `0` and
prints nothing. **That is a silent empty, not an error**, and an editor pointed at the HTTPS `git.`
URL will report something unhelpful rather than "you are not authorized".

Use `ssh://git@git.daveynet.xyz/davey/redextape.git` if you have a key on the instance, or an
...
[[grammar]]
name = "redextape_asm"
source = { git = "https://git.daveynet.xyz/davey/redextape", rev = "<commit sha>", subpath = "grammars/tree-sitter-redextape-asm" }
