...
#[cfg(test)]
use crate::tm::encoding::stack_is_empty;
...
///
/// THE ORDER IS LOAD-BEARING, for the same reason spelled out on `append_field_to_work` below:
/// `seek_slot`/`rewind_home` emit one rule per element of this slice, in slice order, and rule lookup
/// is first-match-wins with no overlap check in `Machine::validate()`. Reversing this to
/// `[BLANK, MARK]` compiles, passes most tests, and silently changes which rule fires — the
/// step-count goldens are what catch it.
...
fn copy_field_to_work(b: &mut Builder, from: StateId, slot: Slot, label: &str) -> StateId {
    ...
    let at = seek_slot(b, after_clear, REG, UNARY_CONTENT, slot, &format!("{label}.s")); // REG at field `slot` first cell
    ...
    let reg_home = rewind_home(b, rin, REG, UNARY_CONTENT, slot, &format!("{label}.r"));
...
fn append_work_to_field(b: &mut Builder, from: StateId, rd: Slot, label: &str) -> StateId {
    let at = seek_slot(b, from, REG, UNARY_CONTENT, rd, &format!("{label}.s")); // REG at field `rd` first cell
    ...
    let reg_home = rewind_home(b, rin, REG, UNARY_CONTENT, rd, &format!("{label}.r"));
    rewind_work(b, reg_home, &format!("{label}.w"))
}
...
fn append_field_to_work(b: &mut Builder, from: StateId, slot: Slot, label: &str) -> StateId {
    ...
    b.add_rule(from, RuleSpec::new().on(WORK, Some(MARK), None, Move::R), from);
    let at = seek_slot(b, from, REG, UNARY_CONTENT, slot, &format!("{label}.s")); // REG at field `slot` first cell
    let cp = b.state(format!("{label}.cp"));
    b.add_rule(at, RuleSpec::new(), cp); // enter the copy loop on the field's first cell
    b.add_rule(cp, RuleSpec::new().on(REG, Some(MARK), None, Move::R).on(WORK, None, Some(MARK), Move::R), cp);
    let rin = b.state(format!("{label}.rin"));
...
fn erase_per_field(b: &mut Builder, from: StateId, rb: Slot, label: &str) -> StateId {
    let at = seek_slot(b, from, REG, UNARY_CONTENT, rb, &format!("{label}.s")); // REG at field `rb` first cell
    ...
    rewind_home(b, rexit, REG, UNARY_CONTENT, rb, &format!("{label}.r")) // WORK already home; just rewind REG
}
...
fn append_field_per_counter(b: &mut Builder, from: StateId, ra: Slot, cnt: Slot, label: &str) -> StateId {
    ...
    let at = seek_slot(b, from, REG, UNARY_CONTENT, cnt, &format!("{label}.s")); // REG at `cnt`'s first cell
    ...
    let reg_home = rewind_home(b, erased, REG, UNARY_CONTENT, cnt, &format!("{label}.r")); // REG home again (WORK untouched)
    ...
    rewind_home(b, done, REG, UNARY_CONTENT, cnt, &format!("{label}.x"))
}
...
impl Encoding for Unary {
    fn write_literal(&self, b: &mut Builder, entry: StateId, exit: StateId, n: u64, rd: Slot) {
        ...
        let at = seek_slot(b, entry, REG, UNARY_CONTENT, rd, &format!("{base}.s"));
        ...
        let home = rewind_home(b, cur, REG, UNARY_CONTENT, rd, &format!("{base}.r"));
    ...
    fn jz(&self, b: &mut Builder, entry: StateId, if_zero: StateId, if_nonzero: StateId, r: Slot) {
        ...
        let at = seek_slot(b, entry, REG, UNARY_CONTENT, r, &format!("{l}.s")); // REG on field r's first cell
        ...
        let home_nz = rewind_home(b, nz, REG, UNARY_CONTENT, r, &format!("{l}.rn"));
        b.add_rule(home_nz, RuleSpec::new(), if_nonzero);
        let home_z = rewind_home(b, z, REG, UNARY_CONTENT, r, &format!("{l}.rz"));
...
mod tests {
    ...
    fn heap_open_then_append_builds_one_cell() {
        ...
        assert_eq!(
            Unary::default().parse_heap_cells(&cells),
            vec![(3, 0)],
            "one cell `@ 3 #` built with an empty tail"
        );
    }
    ...
    fn heap_build_two_cells_and_count() {
        ...
        let (tapes, status) = simulate(&m, &init, TM_DEFAULT_CAPS);
        assert_eq!(status, Status::Halted);
        assert_eq!(enc.parse_heap_cells(&tapes[HEAP].snapshot().0), vec![(3, 0), (2, 1)], "two cells built in order");
    ...
    fn cons_builds_a_cell_and_writes_the_pointer() {
        ...
        fn run_cons(head: u64, tail: u64) -> (Vec<(u64, u64)>, Option<u64>) {
            ...
            let (tapes, status) = simulate(&m, &init, TM_DEFAULT_CAPS);
            assert_eq!(status, Status::Halted);
            (enc.parse_heap_cells(&tapes[HEAP].snapshot().0), enc.decode_nat(&tapes[REG].snapshot().0, 2))
        }
    ...
    fn two_conses_get_sequential_pointers() {
        ...
        let (tapes, status) = simulate(&m, &init, TM_DEFAULT_CAPS);
        assert_eq!(status, Status::Halted);
        assert_eq!(enc.parse_heap_cells(&tapes[HEAP].snapshot().0), vec![(3, 0), (2, 1)]);
